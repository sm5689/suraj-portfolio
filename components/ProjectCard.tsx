...
export function ProjectCard({ repo }: { repo: Project }) {
  ...
  return (
    <div className="group rounded-2xl border border-line bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
      ...
      <div className="mt-4 flex gap-3 text-sm">
        <Link className="text-ink underline-offset-4 hover:underline" href={repo.html_url}>
          GitHub
        </Link>
        {repo.homepage ? (
          <Link className="text-ink underline-offset-4 hover:underline" href={repo.homepage}>
            Live
          </Link>
        ) : null}
      </div>
